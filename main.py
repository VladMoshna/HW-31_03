import re # этот модуль позволяет с помощью шаблонов, он может находить нужные строки либо слова в тексте, проверять формат текста(номер телефона, емейл и тд), разбивать и заменять текст
...
def task2():
    ...
    try:
        with open("log.txt", "r") as f:
            text = f.read().lower()
            found_words = re.findall(r'\b\w+\b', text) # здесь с помощью этой функции создается список из всех слов текста, \b обозначает грантцы слов, \w+ означает одну или больше символов, r перед ковычками говорит интерпретатору что бв он не принимал обратный слеш как специалоьный символ
